...
typedef enum {
    ...
    HID_TASK_VERSION_ATS3607_INDEX,
...
#define HID_TASK_VERSION_ATS3607_MASK                   (1 << HID_TASK_VERSION_ATS3607_INDEX)
...
#define LOG_VERSION_GET                                 L"Version_get.log"
#define LOG_FIRMWARE_UPGRADE                            L"Firmware_upgrade.log"
#define LOG_CONFIGURATION_SET                           L"Configuration_set.log"

#define LOG_DEFAULT_VALUE                               0xFF
#define VERSION_MAJOR_ID                                0x00
#define VERSION_MINOR_ID                                0x18
...
{
    UINT hid_timeout_ms = HID_CONNECTION_TIMEOUT_MS, vid = 0, pid = 0;
...
{
    BOOL        Res_awr = TRUE, Res_awt = TRUE, Res_mcu = TRUE, Res_ats = TRUE;
    BOOL        Res, Ret;
    PUINT       p_version, version_ats = NULL;
    UINT        version_mcu = 0, version_awr = 0, version_awt = 0;
    DEV_CMD_T   MSG = { 0 };

    if (*params->version_mcu || *params->version_awr || *params->version_awt || *params->version_ats)
    {
...
#define ATS3607_VERSION_GET_RETRY_COUNT     1
            ...
            for (UINT i = 0; i < ATS3607_VERSION_GET_RETRY_COUNT; i++)
            {
                ...
                Res_ats = VersionGet(&MSG, VERSION_TYPE_ATS3607);
                if (Res_ats && MSG.payload[VERSION_TYPE_STATUS_INDEX] == VERSION_GET_SUCCESS)
                {
                    debug_printf("Version ATS3607 get: %s\r\n", p_version);
                    version_ats = p_version;
                    break;
                }
                else
                {
                    if ((i + 1) == ATS3607_VERSION_GET_RETRY_COUNT)
                    {
                        debug_printf("Version ATS3607 not found: %X\r\n", MSG.payload[VERSION_TYPE_STATUS_INDEX]);
                        *status |= HID_TASK_VERSION_ATS3607_MASK;
                        Res_ats = FALSE;
                    }
                }
            }
    ...
    log_update:
        ...
        swprintf(file_path, MAX_PATH, L"%ls\\%ls", cwd, LOG_VERSION_GET);
        ...
        if (*params->version_ats != NULL)
            fprintf(fp, "Version ATS3607: %s\r\n", Res_ats ? (PCHAR)version_ats : "0");
        fflush(fp);
        fclose(fp);
    }
}
...
{
    BOOL    isp_arom = *params->isp_aprom == NULL ? FALSE: TRUE, ufp_check = *params->ufp_check == NULL ? FALSE : TRUE, ats3607_mcu_helper = *params->atss == NULL ? FALSE : TRUE;
    ...
    if (*params->awr_file != NULL || *params->awt_file != NULL || *params->isp_file != NULL || *params->ats_file != NULL)
    {
        ...
        if (*params->ats_file != NULL)
        {
            ...
            {
                HidParamsConfig(params);
                ATS3607HelperInit(ats3607_mcu_helper);
                Res_ats = ATS3607HelperRun(*params->ats_file, ats3607_mcu_helper? i < 3? TRUE: FALSE: FALSE);
    ...
    log_update:
        ...
        GetCurrentDirectory(MAX_PATH, cwd);
        swprintf(file_path, MAX_PATH, L"%ls\\%ls", cwd, LOG_FIRMWARE_UPGRADE);
        fp = _wfopen(file_path, L"wb+");
        if (*params->awr_file != NULL)
...
void ConfigurationSet(cmd_parse_s* params, UINT* status)
{
    ...
    log_update:
        ...
        GetCurrentDirectory(MAX_PATH, cwd);
        swprintf(file_path, MAX_PATH, L"%ls\\%ls", cwd, LOG_CONFIGURATION_SET);
        fp = _wfopen(file_path, L"wb+");
...
void TasksStatusShow(cmd_parse_s* params, UINT status_code)
{
    ...
    if (*params->version_ats)
    {
        status = status_code & HID_TASK_VERSION_ATS3607_MASK ? FALSE : TRUE;
        debug_printf(" - Version get ATS3607: %s\r\n", status ? "successful" : "failed");
    }
...
int main(int argc, char* argv[])
{
    ...
    printf("WindowsTool version: %02X.%02X\r\n", VERSION_MAJOR_ID, VERSION_MINOR_ID);
